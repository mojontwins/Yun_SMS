...
extern unsigned char pfacing, psprid;
extern unsigned char pj, pctj, pjb, pgotten, pregotten, ppossee, psprint;
extern unsigned char pobjs, plife, pcontinues, pkeys, pkilled;
extern unsigned char pstate, pctstate, pkilled, phit;
extern signed int pgtmx, pgtmy;
extern unsigned char pfiring, pice, pconveyor, pconvd1, pconvd2;
extern unsigned char pisdead;

extern const unsigned char level_ini_x [];
extern const unsigned char level_ini_y [];

extern signed int px_safe, py_safe;
extern unsigned char n_pant_safe;

extern unsigned char ticks, ticker;
extern unsigned char cx1, cx2, cy1, cy2;
extern unsigned char at1, at2;
extern unsigned char n_pant, on_pant, half_life, level;
extern unsigned char rda, rdx, rdy, prx, pry;
...
inline void player_do (void) {
	...
	if (level < 5) {
		...
		//if (px >= 256 << FIX_BITS) px -= (256 << FIX_BITS);
	}
	
	// Collision
	prx = (unsigned char) (px >> FIX_BITS);
...
inline void player_render_scrolly (void) {
	if (pstate == STATE_NORMAL || half_life) {
		//SMS_MT_addMetaspriteSimple (prx, pry - 17, spr_pl [psprid]);
		SMS_MT_add2x3MetaSpriteSimpleUnrolled (prx - cam_x_module, pry - 17, spr_pl [psprid]);
	}
}
